...
fn test_secrets_vault_roundtrip() {
    ...
    // Create vault
    let mut vault = openpylot::secrets::SecretsVault::open(&path, None).unwrap();
    ...
    // Re-open and verify
    let vault2 = openpylot::secrets::SecretsVault::open(&path, None).unwrap();
    assert_eq!(
...
fn test_secrets_vault_delete() {
    ...
    // Verify
    let vault = openpylot::secrets::SecretsVault::open(&path, None).unwrap();
...
fn test_secrets_has_llm_configured() {
    ...
    let mut vault2 = openpylot::secrets::SecretsVault::open(&path, None).unwrap();
