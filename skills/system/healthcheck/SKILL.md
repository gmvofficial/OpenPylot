...
# pylot Host Hardening

## Overview

Assess and harden the host running pylot, then align it to a user-defined risk tolerance without breaking access. Use pylot security tooling as a first-class signal, but treat OS hardening as a separate, explicit set of steps.
...
- Never claim pylot changes the host firewall, SSH, or OS updates; it does not.
...
5. pylot gateway status and bind address.
...
9. OS automatic security updates status.
   Note: these are not blocking items, but are highly recommended, especially if pylot can access sensitive data.
...
### 2) Run pylot security audits (read-only)

As part of the default read-only checks, run `pylot security audit --deep`. Only offer alternatives if the user requests them:

1. `pylot security audit` (faster, non-probing)
2. `pylot security audit --json` (structured output)

Offer to apply pylot safe defaults (numbered):

1. `pylot security audit --fix`

Be explicit that `--fix` only tightens pylot defaults and file permissions. It does not change host firewall, SSH, or OS update policies.
...
### 3) Check pylot version/update status (read-only)

As part of the default read-only checks, run `pylot update status`.
...
- Credential hygiene notes (location of pylot creds, prefer disk encryption)
...
After pylot install or first hardening pass, run at least one baseline audit and version check:

- `pylot security audit`
- `pylot security audit --deep`
- `pylot update status`

Ongoing monitoring is recommended. Use the pylot cron tool/CLI to schedule periodic audits (Gateway scheduler). Do not create scheduled tasks without explicit approval. Store outputs in a user-approved location and avoid secrets in logs.
...
1. “Do you want me to schedule periodic audits (e.g., daily/weekly) via `pylot cron add`?”

If the user says yes, ask for:

- cadence (daily/weekly), preferred time window, and output location
- whether to also schedule `pylot update status`
...
Before creating, `pylot cron list` and match on exact `name`. If found, `pylot cron edit <id> ...`.
If not found, `pylot cron add --name <name> ...`.

Also offer a periodic version check so the user can decide when to update (numbered):

1. `pylot update status` (preferred for source checkouts and channels)
2. `npm view pylot version` (published npm version)

## pylot command accuracy

Use only supported commands and flags:

- `pylot security audit [--deep] [--fix] [--json]`
- `pylot status` / `pylot status --deep`
- `pylot health --json`
- `pylot update status`
- `pylot cron add|list|runs|run`

Do not invent CLI flags or imply pylot enforces host firewall/SSH policies.
...
Follow the durable-memory prompt format used by pylot compaction:
